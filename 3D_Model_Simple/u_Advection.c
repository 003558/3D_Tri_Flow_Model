 ...
 )
{
  ...
  for(n=0;n<noe;n++){
    for(l=ceG_h[n].ns;l<nol;l++){
      ceR_h[n].u_Adv[l]=((DATA_TYPE)0.0);
      for(e=0;e<3;e++){
	ceR_h[n].u_Adv[l]+=ceG_h[n].sgn[e]*suR_h[ceC_h[n].su[e]].fl[l];
      }
      ceR_h[n].u_Adv[l]/=(ceG_h[n].ds);
    }
  }
  ...
  for(n=0;n<noe;n++){
    ...
    for(l=ceG_h[n].ns;l<nol;l++){
      ceR_h[n].u_Adv[l]+=-(ceR_h[n].fl[l+1]-ceR_h[n].fl[l])/(ceG_h[n].dz[l]*ceG_h[n].ds);

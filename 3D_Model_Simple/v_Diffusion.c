 ...
 )
{
  ...
  for(l=0;l<nol;l++){
    for(n=0;n<nos;n++){
      if(suG_h[n].bd[l]==0){
	dq=-((DATA_TYPE)2.0)*ceR_h[suC_h[n].ce[0]].v[l];
	}
	else{
	dq=ceR_h[suC_h[n].ce[1]].v[l]-ceR_h[suC_h[n].ce[1]].v[l];
      }
  ...
  for(n=0;n<noe;n++){
	...
	ceR_h[n].v_Dif[l]+=(Ev1*dq1/dz1-Ev2*dq2/dz2)/ceG_h[n].dz[l];
      ...
      ceR_h[n].v_Dif[l]+=(Ev1*dq1/dz1-((DATA_TYPE)0.0))/ceG_h[n].dz[l];
      ...
      ceR_h[n].v_Dif[l]+=(((DATA_TYPE)0.0)-Ev2*dq2/dz2)/ceG_h[n].dz[l];
    }
  }
